...
module.exports = async function (app) {
  ...
  app.get(
    process.env.SERVER_PREFIX + "/atividades/detalhe/:codigo",
    function (req, res) {
      let codigoAtividade = req.params.codigo;
      if (!req.session.token) {
        res.redirect(process.env.SERVER_PREFIX + "/app/login");
      } else if (
        req.session.usuario.nivel != "ADMIN" &&
        req.session.usuario.nivel != "ALUNO"
      ) {
        req.flash(
          "danger",
          "Acesso restrito! Somente usuários de nível ADMINISTRADOR ou ALUNO ou PROFESSOR podem acessar esta página"
        );
  ...
  app.get(
    process.env.SERVER_PREFIX + "/app/" + rota + "/list",
    function (req, res) {
      if (!req.session.token) {
        res.redirect(process.env.SERVER_PREFIX + "/app/login");
        ...
        req.flash(
          "danger",
          "Acesso restrito! Somente usuários de nível ADMINISTRADOR podem acessar esta página"
        );
        res.redirect(process.env.SERVER_PREFIX + "/");
      } else {
        teste = request(
  ...
  app.get(
    process.env.SERVER_PREFIX + "/app/" + rota + "/create",
    function (req, res) {
      if (!req.session.token) {
        res.redirect(process.env.SERVER_PREFIX + "/app/login");
      } else if (req.session.usuario.nivel != "ADMIN") {
  ...
  app.post(
    process.env.SERVER_PREFIX + "/app/" + rota + "/create/submit",
    upload.single("photo"),
    function (req, res) {
  ...
  app.get(
    process.env.SERVER_PREFIX + "/app/" + rota + "/edit/:id",
    function (req, res) {
      if (!req.session.token) {
        res.redirect(process.env.SERVER_PREFIX + "/app/login");
      } else if (req.session.usuario.nivel != "ADMIN") {
  ...
  app.get(
    process.env.SERVER_PREFIX + "/app/" + rota + "/perfil",
    function (req, res) {
      if (!req.session.token) {
        res.redirect(process.env.SERVER_PREFIX + "/app/login");
      } else {
  ...
  app.post(
    process.env.SERVER_PREFIX + "/app/" + rota + "/edit/submit",
    upload.single("photo"),
    function (req, res) {
      if (req.file && req.file.size > 220220) {
        req.flash("danger", "Item não salvo. Sua imagem deve ter até 200kb.");
  ...
  app.post(
    process.env.SERVER_PREFIX + "/app/" + rota + "/responder",
    ...
    function (req, res) {
      ...
      request(
        {
          url: process.env.API_HOST + rota + "/responder",
          method: "POST",
          json: true,
          headers: {
            "content-type": "application/json",
          ...
          json: {
            ...
            idAluno: req.session.usuario.id,
        ...
        function (error, response, body) {
          console.log(
            `Aluno ${req.session.usuario.id}, respondendo atividade ${req.body.idAtividade} com resposta ${req.body.resposta}`
          );
  ...
  // Rota para receber parametros via post editar item
  app.post(
    process.env.SERVER_PREFIX + "/app/" + rota + "/perfil/edit/submit",
    upload.single("photo"),
    function (req, res) {
      if (req.file && req.file.size > 220220) {
        req.flash("danger", "Item não salvo. Sua imagem deve ter até 200kb.");
  ...
  app.post(
    process.env.SERVER_PREFIX + "/app/" + rota + "/perfil/alterar-senha/submit",
    upload.single("photo"),
    function (req, res) {
      if (!req.session.token) {
        res.redirect(process.env.SERVER_PREFIX + "/app/login");
  ...
  app.post(
    process.env.SERVER_PREFIX + "/app/" + rota + "/delete/",
    function (req, res) {
      if (!req.session.token) {
        res.redirect(process.env.SERVER_PREFIX + "/app/login");
      } else if (req.session.usuario.nivel != "ADMIN") {
