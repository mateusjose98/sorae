...
$('#unidadeFederativaLotacao').change(function () {
    if ($('#unidadeFederativaLotacao').val() == '') {
        ...
        $("#municipioLotacao").prepend('<option value="">Selecione um Município</option>');
    ...
    $.ajax(
        {
            ...
            success: function (resultado) {
                if (!resultado.data.length) {
                    $('#municipioLotacao').append('<option value="">Nenhum Município</option>');
                    $("#municipioLotacao").prop('disabled', false);
                }
                else {
                    resultado.data.forEach(item => {
                        $("#municipioLotacao").append('<option value="' + item.id + '">' + item.nome + '</option>');
                    });
                    $("#municipioLotacao").prop('disabled', false);
...
statusCheckbox.addEventListener('change', function () {
    if (this.checked) {
        ...
        ufOption.removeEventListener('change', unidadeFederativaParaUnidadeFederativaLotacao);
        municipioOption.removeEventListener('change', municipioParaMunicipioLotacao);
    }
});
...
function municipioParaMunicipioLotacao(event) {
    ...
    document.querySelector('#municipioLotacao').selectedIndex = municipioOption.selectedIndex;
}
function unidadeFederativaParaUnidadeFederativaLotacao(event) {
    CarregaMunicipiosLotacao(ufOption.value, municipioOption.value)
    document.querySelector('#unidadeFederativaLotacao').selectedIndex = ufOption.selectedIndex
...
function passarValores() {
    ...
    ufOption.addEventListener('change', unidadeFederativaParaUnidadeFederativaLotacao);
